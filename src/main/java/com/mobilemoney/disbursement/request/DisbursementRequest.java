...
public class DisbursementRequest extends ViewTransactionRequest {
	...
	public AsyncResponse createBatchTransaction() throws MobileMoneyException {
		...
		if (this.batchTransaction == null) {
			throw new MobileMoneyException(
					new HttpErrorResponse.HttpErrorResponseBuilder(Constants.VALIDATION_ERROR_CATEGORY,
							Constants.VALUE_NOT_SUPPLIED_ERROR_CODE)
									.errorDescription(Constants.DISBURSEMENT_TRANSACTION_OBJECT_INIT_ERROR).build());
		}
		...
		return createRequest(HttpMethod.POST, resourcePath, this.batchTransaction.toJSON(), notificationType,
				callBackURL, AsyncResponse.class);
	}
	 ...
	 * @throws MobileMoneyException
	 */
	public BatchCompletions viewBatchCompletions(final String batchId) throws MobileMoneyException {
	 ...
	 * @throws MobileMoneyException
	 */
	public BatchCompletions viewBatchCompletions(final String batchId, final Filter filter)
			throws MobileMoneyException {
		if (StringUtils.isNullOrEmpty(batchId)) {
			throw new MobileMoneyException(
					new HttpErrorResponse.HttpErrorResponseBuilder(Constants.INTERNAL_ERROR_CATEGORY,
		...
		StringBuilder resourcePath = new StringBuilder(
				API.VIEW_BATCH_TRANSACTION_COMPLETED.replace(Constants.BATCH_ID, batchId));
		...
		if (requestResponse.getPayLoad() instanceof String) {
			completedTransactions = JSONFormatter.fromJSONList((String) requestResponse.getPayLoad(),
					BatchCompletion.class);
		}
		batchCompletions.setBatchCompletions(completedTransactions);
	 ...
	 * @throws MobileMoneyException
	 */
	public BatchRejections viewBatchRejections(final String batchId) throws MobileMoneyException {
	 ...
	 * @throws MobileMoneyException
	 */
	public BatchRejections viewBatchRejections(final String batchId, final Filter filter) throws MobileMoneyException {
		if (StringUtils.isNullOrEmpty(batchId)) {
			throw new MobileMoneyException(
					new HttpErrorResponse.HttpErrorResponseBuilder(Constants.INTERNAL_ERROR_CATEGORY,
							Constants.GENERIC_ERROR_CODE).errorDescription(Constants.NULL_VALUE_ERROR).build());
		...
		StringBuilder resourcePath = new StringBuilder(
				API.VIEW_BATCH_TRANSACTION_REJECTED.replace(Constants.BATCH_ID, batchId));
		...
		batchRejections.setBatchRejections(completedTransactions);
