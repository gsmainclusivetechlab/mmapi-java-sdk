...
public class AccountLinkingRequest extends ViewTransactionRequest {
	...
	/***
	 * 
	 * @param linkJsonString
	 */
	public void setLink(final String linkJsonString) {
		this.link = JSONFormatter.fromJSON(linkJsonString, Link.class);
	}
	...
	public AsyncResponse createAccountLink(Identifiers identifiers) throws MobileMoneyException {
		...
		if (link == null) {
			throw new MobileMoneyException(
					new HttpErrorResponse.HttpErrorResponseBuilder(Constants.VALIDATION_ERROR_CATEGORY,
							Constants.VALUE_NOT_SUPPLIED_ERROR_CODE)
									.errorDescription(Constants.ACCOUNT_LINK_OBJECT_INIT_ERROR).build());
		}
		...
		return createRequest(HttpMethod.POST, resourcePath, link.toJSON(), notificationType, callBackURL,
				AsyncResponse.class);
	}
	...
	public Link viewAccountLink(Identifiers identifiers, String linkReference) throws MobileMoneyException {
		...
		String resourcePath = API.VIEW_ACCOUNT_LINK.replace(Constants.ACCOUNT_ID,
				identifiers.getIdentifiers().stream()
						.map(identifier -> identifier.getKey().concat("@").concat(identifier.getValue()))
						.collect(Collectors.joining("$")))
				.replace(Constants.LINK_REFERENCE, linkReference);
