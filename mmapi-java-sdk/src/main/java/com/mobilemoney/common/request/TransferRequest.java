...
public class TransferRequest extends ViewTransactionRequest {
	...
	public AsyncResponse createQuotation() throws MobileMoneyException {
			...
			throw new MobileMoneyException(
					new HttpErrorResponse.HttpErrorResponseBuilder(Constants.VALIDATION_ERROR_CATEGORY,
							Constants.VALUE_NOT_SUPPLIED_ERROR_CODE)
									.errorDescription(Constants.QUOTATION_OBJECT_INIT_ERROR).build());
		}
		...
		return createRequest(HttpMethod.POST, API.QUOTATION_REQUEST, this.quotation.toJSON(), notificationType,
	...
	/***
	 * 
	 * @param quotationJsonString
	 */
	public void setQuotation(final String quotationJsonString) {
		this.quotation = JSONFormatter.fromJSON(quotationJsonString, Quotation.class);
	}
}
